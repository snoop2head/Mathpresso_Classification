# Moudule Info: https://catsirup.github.io/ai/2020/04/21/nlp_data_argumentation.html

# `` 내부 관련
# 수학 공식과 한글을 바꾸는 것을 막아야 한다.
# ABC 같은 걸 BAC로 바꾸는 걸로 해도 될 것 같은데
# (2,1)을 (3,4)로 바꾸는 등
# 3 -> 4로 단순 교체
# +2 -> -5 로 단순 교체
# A -> B로 단순 교체. 즉 대문자 알파벳을 교체한다. n(수열)이나 p,q(명제)는 교체하지 않는다.
# 대신에 한 문제 내에서 교체한 알파벳/숫자은 그대로 적용. 즉 A -> B이면, 다른 부분에서도 A -> B로 교체
# E, V, P는 확률 단원에서 각각 Expectation, Varsity, Probability에 속하기 때문에 교체하지 않는다.
# ^2나 ^x 등 같은 제곱 다음 첫 글자는 교체하지 않는다


# 아예 같은 문장일 경우 내보내지 않아야 한다.
...
with open(f"{path}/wordnet.pickle", "rb") as f:
...
def get_math_notations(line) -> list:
...
def EDA(sentence, alpha_sr=0.1, alpha_ri=0.1, alpha_rs=0.1, p_rd=0.1, num_aug=9):
    ...
    return list(set_augmented_sentences)
